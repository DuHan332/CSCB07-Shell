...
/**
 * This function would return the inputed commands with indexes which is recorded by the tracker in
 * the interpreter. All inputed commands including errors would be recorded and returned.
 * 
 * @author Du Han
 */
public class History implements Commands {
  ...
  /**
   * This function would call isNumeric to check if input is legal and call the showhistory function
   * with a number which is converted from input. If the number is too big it would throw error
   * 
  ...
  /**
   * This function would return a String that contains recent specific number of inputed commands.
   * this number should be greater than 0, it would throw error if the number is 0.
   * 
  ...
  private static String showhistory(ArrayList<String> list, int number)
      throws SystemErrorException {
    if (number == 0) {
      throw new SystemErrorException("the parameter should be " + "bigger than 0");
