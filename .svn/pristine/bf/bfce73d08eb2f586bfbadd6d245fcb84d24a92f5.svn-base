...
public class RedirectAppend {
   ...
   * path and call the function named redirectAppend to to perform a series of operations.
   * 
  ...
  static public void execute(String inputContent, String path, Tracker tracker)
    ...
    if (path.substring(0, 1).equals("/") == false) {
      redirectAppend(inputContent, filename, fullPath, subPath, tracker.getCurrentDirectory());
    } else {
      redirectAppend(inputContent, filename, fullPath, subPath, tracker.getRootDirectory());
    }
  }
   ...
   * @throws SystemErrorException
   */
  private static void redirectAppend(String content, String fileName, ArrayList<String> fullPath,
