...
public class RemoveDirectory implements Commands {
  ...
  /**
   * This function would call the function named pathToArrayList to get an ArrayList that represents
   * the path, and then it would call a help function named findpath to search the directory that
   * user wants to remove. If the function doesn't find anything wrong, it would remove the specific
   * directory, otherwise it would throw error message.
   * 
  ...
  public String execute(String[] input, Tracker tracker) throws SystemErrorException {
    ...
    if (input[0].equals("/"))
      throw new SystemErrorException("You are not able to delete the root directory");
    ArrayList<String> listOfpath = pathToArrayList(input[0]);
    ...
    } else if (checkDirectory(listOfpath, tracker) == true) {
      JNode node = FindPath.findpath(listOfpath, tracker.getCurrentDirectory());
      if (node != null) {
        if (node.getIsDirectory() == true) {
          node.getRoot().getSub().remove(node);
   ...
   *         directory that user wants to remove
   */
  private boolean checkDirectory(ArrayList<String> listOfpath, Tracker tracker) {
